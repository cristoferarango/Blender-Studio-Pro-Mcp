...
_activity_log = []  # recent command entries for the UI panel
_stats = {"total": 0, "ok": 0, "error": 0, "start_time": 0.0}
_MAX_LOG = 100
...
def _record_activity(command, status, duration_ms):
    """Append a UI log entry and update session counters."""
    _stats["total"] += 1
    if status == "ok":
        _stats["ok"] += 1
    else:
...
import os as _os
_LOG_PATH = _os.path.join(_os.path.expanduser("~"), "blender_studio_pro_mcp.log")


def _log(msg):
    """Debug log to file and Blender console."""
    line = f"[{time.strftime('%H:%M:%S')}] {msg}"
    print(f"[Studio Pro MCP] {msg}")
    try:
...
def _process_queue():
    ...
    _record_activity(cmd_name, status, duration_ms)
...
class BMPRO_OT_ModalServer(bpy.types.Operator):
    ...
    bl_label = "Studio Pro MCP Modal Server"
